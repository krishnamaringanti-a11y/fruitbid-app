import streamlit as stimport matplotlib.pyplot as pltimport pandas as pdimport plotly.express as pxfrom fpdf import FPDFimport base64import osfrom datetime import datetime, timedeltafrom db import get_db_connection, init_db, get_items, get_min_bid, get_market_cap, get_highest_bid, get_billing_rate, get_user_id, get_setting, set_setting, initialize_itemsfrom otp import send_otp, verify_otpfrom nutrition import initialize_nutrition, get_nutrition_datafrom utils import validate_mobile, validate_email, check_admin_password, fetch_real_time_price, monitor_pricesimport tempfile
...
PDF class
class PDF(FPDF):    def header(self):        self.set_font('Arial', 'B', 12)        self.cell(0, 10, 'FruitBid Report', 0, 1, 'C')
def add_content(self, img1_path, img2_path, bid_data, user_data, nutrition_data):
    self.add_page()
    self.set_font('Arial', '', 10)
    self.cell(0, 10, f'Market grows to USD 88.9 billion by 2030. Billing at {get_setting("discount_pct")} percent off cheapest platform. Women farmers lead.', 0, 1)
    self.ln(10)
    for i, row in bid_data.iterrows():
        self.cell(0, 10, f'{row["Item"]} Min {row["Min Bid INR"]} Cap {row["Max Cap INR"]} Highest {row["Highest Bid INR"]} Billing {row[f"Billing Rate INR {get_setting('discount_pct')}pct off"]} Lucky {row["Lucky Dip Winner"]}', 0, 1)
    self.ln(10)
    ...
    for i, row in nutrition_data.iterrows():
        self.cell(0, 10, f'{row["item_name"]}: Calories {row["calories"]}, Fiber {row["fiber"]}g, Vit C {row["vit_c"]}mg, Potassium {row["potassium"]}mg, Notes: {row["notes"]}', 0, 1)
    self.image(img1_path, x=10, y=None, w=90)
    self.image(img2_path, x=110, y=None, w=90)

@st.cache_data(ttl=30)def create_bids_chart(items):    try:        fig, ax = plt.subplots()        highest_bids = [get_highest_bid(item) for item in items]        billing_rates = [get_billing_rate(item) for item in items]        ax.bar(items, highest_bids, alpha=0.5, label='Highest Bid')        ax.bar(items, billing_rates, alpha=0.7, label='Billing Rate')        ax.set_ylabel('INR per kg')        ax.tick_params(axis='x', rotation=45)        ax.legend()        return fig    except Exception as e:        st.error(f"Error generating bids chart: {str(e)}")        return None
@st.cache_data(ttl=300)def create_market_growth_chart():    try:        market_data = pd.DataFrame({            'Year': [2024, 2030],            'Market Size USD Billion': [66.3, 88.9]        })        fig = px.line(market_data, x='Year', y='Market Size USD Billion', markers=True,                      title='Projected Market Growth',                      labels={'Market Size USD Billion': 'USD Billion'})        fig.update_traces(hovertemplate='Year: %{x}Size: %{y} USD Billion')        fig.update_layout(xaxis_title='Year', yaxis_title='Market Size USD Billion')        return fig    except Exception as e:        st.error(f"Error generating market chart: {str(e)}")        return None
def main():    """Main function to run the FruitBid Streamlit app."""    st.title('FruitBid')    st.header('Bid on Fresh Items')    st.markdown("""        India market grows to USD 66.3 billion in 2024 reaching USD 88.9 billion by 2030.        Online sales surged 143 percent from 2019 to 2020. Women farmers lead.        Bid on fruits, vegetables, or more. Max bid at market rate. Billing at custom discount off cheapest from JioMart, Amazon, Big Bazaar.        Register with mobile or email and address. Bids run three days with delivery on day four. Lucky dip awards free items.    """)
...
with st.form('otp_form'):
    otp_input = st.text_input('Enter OTP', help="Enter the 4-digit OTP received.")
...
if submit_otp and 'temp_reg' in st.session_state:
    if verify_otp(st.session_state.temp_reg['reg_id'], otp_input):
        ...
        if conn:
            try:
                c = conn.cursor()
                c.execute("INSERT INTO users (mobile_email, address, verified) VALUES (?, ?, 1)", (data['reg_id'], data['address']))
                conn.commit()
...
# Monitor billing and lucky dip
if not bidding_open:
    billing_calculated = get_setting('billing_calculated')
    if billing_calculated != bid_start_str:
        conn = get_db_connection()
        if conn:
            try:
                ...
                set_setting('billing_calculated', bid_start_str)
...
if st.session_state.admin_logged:
    with st.expander("Admin Manage Items and Discount"):
        with st.form('item_form'):
            ...
            discount_pct = st.number_input('Discount Percent for Cycle', min_value=0.0, max_value=100.0, step=1.0, value=float(get_setting('discount_pct')))
    ...
    with st.expander("Admin Update Minimum Bids"):
        ...
        with st.form('min_bids_form'):
            new_min_bids = {}
            for item in items:
                new_min_bids[item] = st.number_input(f'Minimum Bid for {item} INR per kg', value=get_min_bid(item), min_value=0.0, step=1.0, key=f'min_{item}')
                ...
                try:
                    ...
                    with st.form('nutrition_form'):
                        ...
                        if st.form_submit_button('Update Nutrition'):
                            c.execute("INSERT OR REPLACE INTO nutrition (item_name, calories, fiber, vit_c, potassium, notes) VALUES (?, ?, ?, ?, ?, ?)",
...
if bidding_open and st.session_state.current_user:
    ...
    cap = get_market_cap(item)
    min_bid = get_min_bid(item)
    current_highest = get_highest_bid(item)
    ...
    if st.button('Submit Bid'):
        ...
        if conn:
            try:
                c = conn.cursor()
                c.execute("SELECT bid_amount FROM bids WHERE item_name=? AND user_id=? ORDER BY bid_amount DESC LIMIT 1", (item, st.session_state.current_user_id))
                existing = c.fetchone()
                if existing and bid_amount <= existing[0]:
                    ...
                    c.execute("INSERT INTO bids (item_name, user_id, bid_amount, timestamp) VALUES (?, ?, ?, ?)",
...
if conn:
    try:
        for item in items:
            ...
            billing_rate = get_billing_rate(item)
            c = conn.cursor()
            c.execute("SELECT u.mobile_email, ld.bid_amount FROM lucky_dip ld JOIN users u ON ld.user_id = u.id WHERE ld.item_name=?", (item,))
...
try:
    ...
    market_data = pd.DataFrame({
        'Year': [2024, 2030],
        'Market Size USD Billion': [66.3, 88.9]
    })
